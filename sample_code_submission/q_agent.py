...
class Agent():
    ...
    def meta_train(self, datasets_meta_features, algorithms_meta_features, train_learning_curves,
        ...
        self.Q = np.random.rand(10, len(self.p_portions) - 1, self.n_actions)
        maxit = 3000
    ...
    def suggest_for_train(self, observation):
        ...
        if observation == None:
            ...
            if self.used_times > 2 * self.total_time_budget / 3:
                time_used = t
                weight = (self.total_time_budget - time_used) / self.total_time_budget
                weight = weight if weight > 0 else 0.0

                reward = (np.max([R_validation_A_p, self.old_score]) - self.old_score) * weight
                ...
                if self.used_times > self.time_budget * 0.2:
                    self.time_used += 1
            else:
    ...
    def kmeans_clustering(self, datasets_meta_features):
        ...
        kmeans = KMeans(n_clusters=10, random_state=0).fit(ds_features)
