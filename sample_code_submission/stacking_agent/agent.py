...
class Agent():
    ...
    def __init__(self, number_of_algorithms):
        ...
        self.model = [RandomForestRegressor(), ExtraTreesRegressor(), RandomForestRegressor(), ExtraTreesRegressor(),
                      RandomForestRegressor()]
        self.model1 = [RandomForestClassifier(), ExtraTreesClassifier(), RandomForestClassifier(),
    ...
    def get_times_class(self, times, time_budget):
        ts = []
        for i in times:
            if i / time_budget < 0.1:
                t = 0
            elif 0.1 <= i / time_budget < 0.2:
                t = 1
            elif 0.2 <= i / time_budget < 0.3:
                t = 2
            elif 0.3 < i / time_budget <= 0.5:
                t = 3
            else:
                t = 4
            ts.append(t)
    ...
    def reset(self, dataset_meta_features, algorithms_meta_features):
        ...
        for algo_name in self.algo_name_list:
            ...
            times = self.voting1.predict(pdata)
        ...
        for i in range(len(self.data_sizes)):
            ...
            best_idxs = np.argsort(scores)[:-3: -1]

            self.best_algo_list += list(best_idxs)
        ...
        self.best_algo_runtime_sum = [sum(list(self.times_list[i])[:3]) for i in self.best_algo_list]
        ...
        self.best_algo_list = [a[0] for a in algo_list][:3]
    ...
    def meta_train(self, datasets_meta_features, algorithms_meta_features, train_learning_curves,
        ...
        for key, ds in validation_learning_curves.items():
            ...
            for algo_name in self.algo_name_list:
                ...
                for sizes in ds[algo_name].training_data_sizes:
                    inputs_list1.append(ds_vector + algo_hp_value_dict1[algo_name] + [sizes])
                ...
                score_list += list(ds[algo_name].scores)

                # times_list += self.get_times_class(new_times)
                times_list += self.get_times_class(list(ds[algo_name].times), ds[algo_name].time_budget)
        ...
        #     self.model[i].fit(x1_data, y1_data)
        #     pred = self.model[i].predict(x1_data)
        #     loss = mean_squared_error(y1_data, pred)
        #     print(i, ":", loss)
        ...
        for i, d in enumerate(kf.split(x1_data, y1_data)):
            ...
            pred = self.model[i].predict(x1_data[[test_index]])
            loss = mean_squared_error(y1_data[test_index], pred)
            print(i, ":", loss)

        # for i in range(self.model_nums):
        #     self.model1[i].fit(x1_data, y2_data)
        #     pred = self.model1[i].predict(x1_data)
        #     f1 = f1_score(y2_data, pred, average="micro")
        #     print(i, ":", f1)

        for i, d in enumerate(kf.split(x1_data, y2_data)):
            train_index = list(d[0])
            test_index = list(d[1])
            self.model1[i].fit(x1_data[train_index], y2_data[train_index])
            pred = self.model1[i].predict(x1_data[test_index])
            loss = mean_squared_error(y2_data[test_index], pred)
            print(i, ":", loss)

        self.voting = VotingRegressor(estimators=[("Model_" + str(i), self.model[i]) for i in range(self.model_nums)])
        self.voting1 = VotingClassifier(
            estimators=[("Model_" + str(i), self.model1[i]) for i in range(self.model_nums)])
        self.voting.fit(x1_data, y1_data)
        # r_p = self.voting.predict(x1_data)
        # print(mean_squared_error(y1_data, r_p))

        self.voting1.fit(x1_data, y2_data)
        # c_p = self.voting1.predict(x1_data)
        # print(f1_score(y2_data, c_p, average="micro"))
    ...
    def suggest(self, observation):
        ...
        if self.time_used < self.total_budget // 4 or max(self.validation_last_scores) <= 0.1:
            ...
            p_index = self.scores_list[next_algo_to_reveal].index(max(self.scores_list[next_algo_to_reveal]))
            if np.random.uniform() > 0.1:
                if self.times_list[next_algo_to_reveal][p_index] >= 4:
                    try:
                        p_index = list(self.times_list[next_algo_to_reveal]).index(3)
                    except:
                        p_index = self.delta_index_list[next_algo_to_reveal]
                        self.delta_index_list[next_algo_to_reveal] += 1
            p = new_budget[p_index]
